...
struct AddressHasher {
  size_t operator()(const Address& address) const {
    const size_t prime = 303053;
    return s_hash(address.city) * prime * prime + s_hash(address.street) * prime + i_hash(address.building);
  }
...
struct PersonHasher {
  size_t operator()(const Person& person) const {
    const size_t prime = 300869;
    return s_hash(person.name) * pow_(prime, 3) + 
           i_hash(person.height) * pow_(prime, 2) + 
           d_hash(person.weight) * prime + 
           address_hash(person.address);
  }

private:
  size_t pow_(size_t base, size_t exp) const {
    size_t cur = base;
