...
public:
  ...
  HttpResponse& AddHeader(string name, string value) {
    headers_[move(name)].push_back(move(value));
    return *this;
  }
...
private:
  HttpCode code_;
  string content_;
  map<string, vector<string>> headers_;
};
...
ostream& operator << (ostream& output, const HttpResponse& resp) {
  ...
  output << '\n';
  for (const auto& [header_name, values] : resp.headers_) {
  ...
  HttpResponse ServeRequest(const HttpRequest& req) {
    if (req.method == "POST") {
      ...
      if (req.path == "/user_comments") {
        ...
        const string captcha_response = "What's the answer for The Ultimate Question of Life, the Universe, and Everything?";
        return HttpResponse(HttpCode::Ok).SetContent(captcha_response);
      } else {
        return HttpResponse(HttpCode::NotFound);
      }
    }
