...
Json::Node GetStopInfoRequest::Process(const TransportManager& manager) const {
  ...
  result_map["buses"] = Json::Node(move(buses_array));

  return Json::Node(move(result_map));
}

GetRouteRequest::GetRouteRequest() : ReadRequest(Type::READ_ROUTE) {}

void GetRouteRequest::ParseFrom(std::string_view input) {
  // TO DO
}

void GetRouteRequest::ParseFrom(const Json::Node& request_node) {
  const auto& request_info = request_node.AsMap();
...
Json::Node GetRouteRequest::Process(const TransportManager& manager) const {
  using WaitActivity = TransportManager::WaitActivity;
  using BusActivity = TransportManager::BusActivity;

  map<string, Json::Node> result_map;
  std::optional<TransportManager::BuildedRouteInfo> stats = manager.GetRoute(from, to);
  ...
  result_map["total_time"] = (*stats).total_time;
  vector<Json::Node> items_array;
  items_array.reserve((*stats).items.size());
  for (variant<WaitActivity, BusActivity>& item : (*stats).items) {
    map<string, Json::Node> item_map;
    if (holds_alternative<WaitActivity>(item)) {
      auto& item_ref = get<WaitActivity>(item);
      item_map["type"] = Json::Node("Wait"s);
      item_map["time"] = Json::Node(static_cast<int>(item_ref.time));
      item_map["stop_name"] = Json::Node(item_ref.stop_name);
    } else if (holds_alternative<BusActivity>(item)) {
      auto& item_ref = get<BusActivity>(item);
      item_map["type"] = Json::Node("Bus"s);
      item_map["bus"] = Json::Node(item_ref.bus);
      item_map["span_count"] = Json::Node(static_cast<int>(item_ref.span_count));
      item_map["time"] = Json::Node(item_ref.time);
    }
    items_array.push_back(Json::Node(move(item_map)));
  }
  result_map["items"] = Json::Node(move(items_array));
...
optional<vector<Json::Node>> ProcessJsonRequests(const std::vector<RequestHolder>& requests, TransportManager& manager) {
  ...
  for (const auto& request_holder : requests) {
    if (request_holder->type == Request::Type::READ_BUS) {
      ...
      results.push_back(request.Process(manager));
    } else if (request_holder->type == Request::Type::READ_ROUTE) {
      const auto& request = static_cast<const GetRouteRequest&>(*request_holder);
      results.push_back(request.Process(manager));
    } else {
