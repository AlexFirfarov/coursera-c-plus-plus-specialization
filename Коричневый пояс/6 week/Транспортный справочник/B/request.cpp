...
void GetStopInfoRequest::Process(const TransportManager& manager) const {
  optional<const TransportManager::StopStats*> result = manager.GetStopStats(stop_name);
  if (!result) {
    cout << "Stop " << stop_name << ": not found\n";
    return;
  }
  if ((*result)->bus_names.empty()) {
    cout << "Stop " << stop_name << ": no buses\n";
    return;
  }
  cout << "Stop " << stop_name << ": buses";
  for (const string& bus : (*result)->bus_names) {
    cout << " " << bus;
  } 
  cout << '\n';
}
