...
public:
  ...
  explicit ConcurrentMap(size_t bucket_count) 
    : data(bucket_count), mutexes(bucket_count) {}

  WriteAccess operator[](const K& key) {
    size_t index = hasher(key) % data.size();
    return {Lock(mutexes[index]), data[index][key]};
  }
  
  ReadAccess At(const K& key) const {
    size_t index = hasher(key) % data.size();
    return {Lock(mutexes[index]), data[index].at(key)};
  }

  bool Has(const K& key) const {
    size_t index = hasher(key) % data.size();
    auto g = Lock(mutexes[index]);
  ...
  MapType BuildOrdinaryMap() const {
    ...
    for (size_t i = 0; i < data.size(); ++i) {
      auto g = Lock(mutexes[i]);
      result.insert(data[i].begin(), data[i].end());
    }
    return result;
  }
