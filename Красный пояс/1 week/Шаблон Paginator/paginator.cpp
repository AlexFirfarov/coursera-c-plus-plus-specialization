...
public:
    Paginator(Iterator begin, Iterator end, size_t page_size) {
        size_t distance_to_end = std::distance(begin, end);
        //pages.reserve(std::ceil(distance_to_end / page_size));

        Iterator it = begin;
        while (distance_to_end) {
            size_t cur_page_size = std::min(page_size, distance_to_end);
            pages.emplace_back(it, std::next(it, cur_page_size));
            distance_to_end -= cur_page_size;
            std::advance(it, cur_page_size);
        }
    }
    ...
    class Page {
    public:
        Page(Iterator begin, Iterator end) : first(begin), last(end), page_size(std::distance(begin, end)) {}
